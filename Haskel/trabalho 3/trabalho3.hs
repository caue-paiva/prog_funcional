...
segmentoCrescAux :: [Int] -> Int  -> Int -> Int --função auxiliar que implementa a lógica do programa
segmentoCrescAux []  maiorSequencia _ = maiorSequencia -- caso base de uma lista vazia, retorna a maior sequencia
segmentoCrescAux [anterior]  maiorSequencia sequenciaAtual = max maiorSequencia sequenciaAtual --caso base de um item sobrando, compara a sequencia atual com a maior sequencia e pega o max
segmentoCrescAux (anterior:atual:resto)  maiorSequencia sequenciaAtual -- caso normal da recursão,    
  | atual > anterior = let sequenciaAtualizada = sequenciaAtual + 1 --se o atual é maior que o anterior, cria um var com a sequencia antiga + 1
                       in if sequenciaAtualizada > maiorSequencia --se a sequencia atualizada é maior que a maior sequencia chama a função com essa sequencia como maiorSequencia
                            then segmentoCrescAux (atual:resto)  sequenciaAtualizada sequenciaAtualizada
                            else segmentoCrescAux (atual:resto)  maiorSequencia sequenciaAtualizada --senão, chama a função com a maiorSequencia anterior
  | otherwise = segmentoCrescAux (atual:resto)  maiorSequencia 1 --caso o atual n seja maior que o anterior, reseta a sequencia atual para 1
...
segmentoCrescente (x:xs) = segmentoCrescAux (x:xs)  1 1 --Caso normal, chama a função aux
